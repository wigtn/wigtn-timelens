...
1. **실시간 AI 큐레이터** — 박물관 유물, 회화, 조각상에 카메라를 향하면, 세계 최고 수준 박물관 도슨트의 깊이를 가진 즉각적인 음성 해설 제공
...
| 전통적 오디오 가이드 | 사전 녹음 해설 | 고정 스크립트, 후속 질문 불가, 시각적 보강 없음 | 인터럽트 가능한 동적 대화 + 시각적 유물 복원 |
| **경쟁사 없음** | 유물 복원 | 손상된 유물을 실시간으로 시각적 복원하는 기존 제품 없음 | **유일무이: 깨진 → 완전, 색바랜 → 선명 (AI 이미지 생성)** |

### 1.5 트랙 전략: Best Overall Agent

"Best Overall Agent" 트랙이 우선시하는 것:
1. **Gemini 기능 사용 폭** — TimeLens는 9개 기능 사용 (최대 커버리지)
2. **에이전트 정교함** — ADK 멀티에이전트: 4개 전문 에이전트 + 오케스트레이터
3. **실세계 유용성** — 전 세계 10만+ 박물관과 연간 14억 유산 방문객의 실제 문제 해결
4. **프로덕션 준비도** — GCP 배포, URL 접근 가능, 모바일 최적화
...
- **모델**: `gemini-2.0-flash-live` (또는 최신 Live API 모델)
...
- **프로토콜**: REST (요청/응답)
- **모델**: `gemini-2.5-flash-preview-image-generation` (또는 최신 이미지 생성 모델)
- **입력**: 텍스트 프롬프트 + 선택적 참조 이미지 (base64)
- **출력**: 생성된 이미지 (PNG/JPEG) + 텍스트
...
| **데모 영상** | 커버됨 | 섹션 9 스크립트에 따른 4분 영상 |

### 8.2 심사 기준 매핑

| 기준 | 가중치 | TimeLens 전략 | 목표 점수 |
|---|---|---|---|
| **Gemini API 활용** | 높음 | 9개 기능 사용 (가능한 최대 커버리지) | 9/10 |
| **에이전트 정교함** | 높음 | ADK 멀티에이전트: 4개 전문가 + 오케스트레이터, 듀얼 파이프라인, Function Calling Bridge | 9/10 |
| **창의성 & 혁신** | 중간 | 박물관 유물 복원 (고유함 — 경쟁사 없음), 문화유산 니치 | 9/10 |
| **실세계 유용성** | 중간 | 전 세계 10만+ 박물관; 설치 없는 웹앱; 다국어; 어떤 유물에든 작동 | 8/10 |
| **기술적 실행** | 높음 | GCP 프로덕션 배포, WebSocket 스트리밍, 반응형 모바일 UI | 8/10 |
| **데모 품질** | 높음 | 킬러 모먼트(Before/After 슬라이더)가 포함된 4분 스크립트 데모 | 9/10 |
...
| **AI 플랫폼** | Gemini API | 2.5 Flash / 2.0 Flash Live | 듀얼 파이프라인 AI 기능 |
...
**목표**: Best Overall Agent ($30K 대상)

**차별화 핵심**:

1. **기능 수 리더십**: 9개 Gemini 기능 (경쟁자 중 최다 가능성 높음)
2. **시각적 "와우" 모먼트**: 손상된 유물 → 깨끗한 복원을 Before/After 슬라이더로 — 즉시 이해 가능하고 감정적으로 강렬함. 심사위원이 기억함
3. **박물관 퍼스트 = 미개척 니치**: 박물관에서 실시간 AI 유물 복원 + 음성 대화를 하는 기존 제품 없음. 야외 유산 앱은 있음 (Google Arts & Culture); 박물관 유물 복원은 없음
4. **실제 문제, 실제 사용자**: 전 세계 10만+ 박물관, 불충분한 라벨 뒤 유리 안에 있는 수십억 유물
5. **제로 마찰**: URL → 카메라 → 10초 이내 AI와 대화
6. **멀티에이전트 정교함**: 단순 챗봇이 아님; 파이프라인 오케스트레이션을 가진 4개 전문 에이전트
7. **프로덕션 품질**: GCP 배포, 반응형 모바일 UI, 적절한 에러 핸들링

**경쟁자 공통 약점 활용**:
- 대부분의 해커톤 출품작은 2-3개 Gemini 기능 사용 → TimeLens는 9개
