...
# Live API, Image Generation에 사용 (서버 전용)
GOOGLE_GEMINI_API_KEY=

# ADK에서 사용하는 API Key (GOOGLE_GEMINI_API_KEY와 동일 값)
# 주의: ADK TypeScript는 반드시 이 이름을 사용 (GOOGLE_API_KEY 아님)
...
1. `GOOGLE_GEMINI_API_KEY`와 `GOOGLE_GENAI_API_KEY`는 동일한 키를 사용하지만, SDK가 기대하는 환경 변수명이 다름
   - `@google/genai`: `GOOGLE_GEMINI_API_KEY` (코드에서 직접 읽어 사용)
   - `@google/adk`: `GOOGLE_GENAI_API_KEY` (ADK가 자동으로 읽음)
...
export async function GET(): Promise<NextResponse<HealthResponse>> {
  const services = {
    liveApi: !!process.env.GOOGLE_GEMINI_API_KEY,
    imageGen: !!process.env.GOOGLE_GEMINI_API_KEY,
...
3. **`NEXT_PUBLIC_*` build-arg**:
   ...
   - 서버 전용 변수 (`GOOGLE_GEMINI_API_KEY` 등)는 런타임 환경 변수로 Cloud Run에서 설정
...
steps:
  ...
  - name: 'gcr.io/google.com/cloudsdktool/cloud-sdk'
    ...
    args:
      ...
      - 'NODE_ENV=production'
      - '--set-secrets'
      - 'GOOGLE_GEMINI_API_KEY=GOOGLE_GEMINI_API_KEY:latest,GOOGLE_GENAI_API_KEY=GOOGLE_GENAI_API_KEY:latest,FIREBASE_SERVICE_ACCOUNT_KEY=FIREBASE_SERVICE_ACCOUNT_KEY:latest,GOOGLE_PLACES_API_KEY=GOOGLE_PLACES_API_KEY:latest'
...
4. **런타임 시크릿 (`--set-secrets`)**:
   ...
   - `GOOGLE_GEMINI_API_KEY`, `GOOGLE_GENAI_API_KEY`, `FIREBASE_SERVICE_ACCOUNT_KEY`, `GOOGLE_PLACES_API_KEY`
5. **Cloud Run 설정**:
...
|---|---|---|
| `src/lib/gemini/client.ts` | `process.env.GOOGLE_GEMINI_API_KEY` | `.env.example`에 정의 |
